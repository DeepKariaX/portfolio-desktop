<script lang="ts">
  ...
  // Global sound volume store (0-100)
  export const soundVolume = writable(40);

  // Update all audio/video elements on the page when soundVolume changes
...
<style>
  :global([data-control-center] .toggle-accent svg) {
    color: #fff !important;
    fill: #fff !important;
  }
  :global([data-control-center] svg) {
    color: var(--accent-gradient, #0fa);
    fill: var(--accent-gradient, #0fa);
  }
</style>
...
>
  <div class="bg-control col-span-2 col-start-1 flex h-33 flex-col justify-center gap-2 pl-3">
    <ControlCenterToggle primary="Wi-Fi" secondary="deepkaria.dev" disabled>
