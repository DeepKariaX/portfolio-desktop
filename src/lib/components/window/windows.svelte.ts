...
export function snap(id: string, snap: WindowSnap, target?: HTMLElement): void {
  ...
  windows[index].previousPosition = fromTranslate(targetWindow);
...
export function unsnap(id: string): Vector | undefined {
  ...
  if (window.previousSize && container.current !== null) {
    ...
    if (window.previousPosition) {
      move(element, window.previousPosition.x, window.previousPosition.y, 0);
    } else {
      const containerSize = getSize(container.current);
      const x = (containerSize.x - window.previousSize.x) / 2;
      const y = (containerSize.y - window.previousSize.y) / 2;
      move(element, x, y, 0);
    }
