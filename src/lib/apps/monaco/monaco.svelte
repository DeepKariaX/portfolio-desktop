<script lang="ts">
  ...
  const app = applications.find((app) => app.id === "dev.deepkaria.monaco");
...
{#if dialogOpen}
  <WindowDialog>
    ...
    <label class="grid grid-cols-[1fr_3fr] items-center gap-2">
      ...
      <input type="text" class="input justify-self-start" bind:value={filename} />
    </label>
    <div class="mt-3 flex w-full items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-2">
        ...
        >
        <button class="btn btn-accent w-20" onclick={downloadFile}>Save</button>
      </div>
    </div>
  </WindowDialog>
{/if}
